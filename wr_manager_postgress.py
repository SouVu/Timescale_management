...
def get_df(query, params=None):
    ...
    try:
        ...
        st.warning(f"Refreshed database structure... ({e})")
...
def init_db():
    ...
    with conn.cursor() as cur:
        ...
        # --- AUTO-MIGRATIONS (Fixes KeyError) ---
        # This forces the new columns to be added to your OLD tables
...
# --- 4. APP ENTRY ---
st.set_page_config(layout="wide", page_title="WR Manager V3")
init_db()  # <--- This now fixes the missing columns automatically
...
if projects.empty:
    ...
    p_name = st.sidebar.text_input("New Network Name")
    if st.sidebar.button("Create Network"):
        run_query("INSERT INTO projects (name) VALUES (%s)", (p_name,))
        st.rerun()
    st.stop()

sel_p_name = st.sidebar.selectbox("Active Network", projects['name'])
pid = int(projects[projects['name'] == sel_p_name]['id'].values[0])

# Custom Schema Logic
schema_raw = projects[projects['id']==pid]['custom_schema'].values[0]
current_schema = schema_raw if schema_raw else {}

with st.sidebar.expander("🛠️ Custom Fields"):
    st.write("Add fields (e.g., 'Jitter', 'Location')")
    ...
    if st.button("Add Field"):
        ...
        if new_field_name and new_field_name not in current_schema[key]:
            ...
            st.rerun()
    st.write("Current:", current_schema)
